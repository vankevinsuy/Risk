...
//					current_player.AddTank(10, new Pion_Tank(current_player.getCheminicopionTank(), new Zone("Asie", 1, new Color(227, 180, 78))));
					...
					for (int j2 = 0; j < sliderSoldat.getValue(); j2++) {
						listePionAttaquantSoldat.get(j2).getZone().setNum_zone(zonenumDefenseur);
						current_player.getListe_de_pion_soldat().get(j2).getZone().setNum_zone(zonenumDefenseur);
					}
					for (int j2 = 0; j < slider_Cavalier.getValue(); j2++) {
						listePionAttaquantCavalier.get(j2).getZone().setNum_zone(zonenumDefenseur);
						current_player.getListe_de_pion_cavalier().get(j2).getZone().setNum_zone(zonenumDefenseur);

					}
					for (int j2 = 0; j < slider_Tank.getValue(); j2++) {
						listePionAttaquantTank.get(j2).getZone().setNum_zone(zonenumDefenseur);
						current_player.getListe_de_pion_tank().get(j2).getZone().setNum_zone(zonenumDefenseur);
